...
function Obog() {
  ...
  const [otherMembers, setOtherMembers] = useState([]); // Supabaseから取得するメンバーを格納
  const [loading, setLoading] = useState(true); // データのロード状態

  // 固定の幹事メンバーリスト
  const kanjiMembers = [
    { id: 'kanji-1', name: "服部真也斗", role: "Beyond Alumni 幹事メンバー" },
    { id: 'kanji-2', name: "遠藤竜仁", role: "Beyond Alumni 幹事メンバー" },
    { id: 'kanji-3', name: "平岡千香子", role: "Beyond Alumni 幹事メンバー" },
  ];
  ...
  useEffect(() => {
    const fetchOtherMembers = async () => {
      try {
        setLoading(true);
        // Supabaseの'members'テーブルからデータを取得
        const { data, error } = await supabase
          .from('members') // テーブル名を指定
          .select('*'); // 全てのカラムを選択

        if (error) {
          throw error;
        }
        setOtherMembers(data); // 取得したデータをotherMembersステートにセット
      } catch (error) {
        console.error("Error fetching other members:", error.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white shadow-md rounded-lg">
      ...
      <h3 className="mt-6 text-xl font-semibold text-gray-800">その他のメンバー</h3>
      {loading ? (
        <p>その他のメンバーをロード中...</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-2xl mx-auto">
      ...
      {isOpen && selectedMember && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg">
            ...
            {/* ここに詳細情報を追加できます（例: selectedMember.description） */}
            <button
